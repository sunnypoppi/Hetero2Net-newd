...
from hetero2net.layers_3 import DisenConv
...
class HeteroGNN(nn.Module):
    def __init__(self, metadata, in_channels, hidden_channels, 
        ...
        if isinstance(in_channels, int):
            self.in_channels_dict = {node_type: in_channels for node_type in metadata[0]}
        elif isinstance(in_channels, dict):
            self.in_channels_dict = in_channels
        else:
            raise ValueError("in_channels must be an int or dict of {node_type: dim}")
        ...
        for i in range(num_layers):
            ...
            # 用 edge_type 构造每个 DisenConv，传入节点维度
            conv = ModuleDict({
                '__'.join(edge_type): layer(
                    (
                        self.in_channels_dict[edge_type[0]],
                        self.in_channels_dict[edge_type[2]]
                    ),
                    second_channels
                )
    ...
    def hetero_conv(self, layer, x_dict, edge_index_dict):
        ...
        for edge_type, edge_index in edge_index_dict.items():
            ...
            #  新写法：传入 src_type 和 dst_type
            if src == dst:
                out = conv(x_dict[src], edge_index, src_type=src, dst_type=dst)
            else:
                out = conv((x_dict[src], x_dict[dst]), edge_index, src_type=src, dst_type=dst)
